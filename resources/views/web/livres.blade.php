...
<main >
    <section class="results-header">
        ...
        <div class="results-header__option">
            <div class="option__button option--grid selected"><span></span><span></span><span></span><span></span><span></span></div>
            <div class="option__button option--list"><span></span><span></span><span></span><span></span></div>
        </div>
    </section>
...
$(".option__button").on("click", function() {
	...
	if ($(this).hasClass("option--grid")) {
     ...
     function loadCategories(){
      var url = "{!! url('/web-api-categorie-resource') !!}";
      var public_path = "{!! url('') !!}";
      var edit_path = "";
      $('.filters').replaceWith('<div class="filters"></div>');
     var divi =  '<div class="filters__item">'+
                                '<div class="checkbox">'+
                                    '<a href="javascript: void(0)" onclick="loadBooks()">'+
                                    '<label for="checkbox-1">Tous<span class="box"></span>'+
    ...
    $('.filters').append(divi);
      ...
      axios.get(url).then((response) =>{
          ...
          categories.forEach(item => {
                ...
                div +=  '<div class="filters__item">'+
                                '<div class="checkbox">'+
                                    '<a href="javascript: void(0)" onclick="loadBooksByCategorie('+item.id+')">'+
                                    '<label for="checkbox-1">'+item.nom+'<span class="box"></span>'+
                                        '<div class="tooltip top">'+
                                            '<span><i class="icon-info"></i></span>'+
                                        '</div>'+
                                    '</label></a>'+
                                '</div>'+
                                    '<span class="badge status-primary">'+item.livres.length+'</span>'+
                            '</div>';
          });
     ...
     function loadBooks(){
      ...
      var public_path = "{!! url('') !!}";
      var edit_path = "";
      $('.results-section').replaceWith('<div class="results-section results--grid"></div>');
      ...
      axios.get(url).then((response) =>{
          ...
          livres.forEach(item => {
              ...
              livre += '<div class="profile">'+
                            '<div class="profile__image"><img src="uploads/livres/'+item.couverture+'" alt="Doggo" /></div>'+
                            '<div class="profile__info">'+
                                '<h3'+item.titre+'</h3>'+
                                '<p class="profile__info__extra">'+item.resume+'</p>'+
                            '</div>'+
                            '<div class="profile__stats">'+
                                '<p class="profile__stats__title">Auteur</p>'+
                                '<h5 class="profile__stats__info">'+item.auteur+'</h5>'+
                            '</div>'+
                            '<div class="profile__stats">'+
                                '<p class="profile__stats__title">Date de publication </p>'+
                                '<h5 class="profile__stats__info">'+item.date_publication+'</h5>'+
                            '</div>'+
                            '<div class="profile__cta"><a class="btn btn-primary" style="color: #ffffff;">Lire</a></div>'+
                        '</div>';
          });

            $('.results-section').append(livre);
      });
     }

     function loadBooksByCategorie(categorie){
        var url = "{!! url('/web-api-load-livres') !!}/"+categorie;
      var public_path = "{!! url('') !!}";
      var edit_path = "";
      $('.results-section').replaceWith('<div class="results-section results--grid"></div>');
